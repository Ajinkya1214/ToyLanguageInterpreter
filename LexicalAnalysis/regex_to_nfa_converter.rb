...
def its_a_match(nfa, s)
  curr_states = get_start_states_from_nfa(nfa)

  s.each_char do |char|
    q = []
    curr_states.each do |state|
      temp = state.state_transitions[char]
      next if temp == nil
      q.unshift(temp)
    end
  ...
  curr_states.each do |state|
    return true if state.is_accepting
  end
  false
end

def its_a_match_2(nfa, s)
  ...
  accepting = is_any_accepting(curr_states)
  if accepting != nil
    return true
  end
  false
